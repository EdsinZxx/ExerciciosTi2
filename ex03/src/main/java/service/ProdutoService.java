...
public class ProdutoService {
	...
	public Object insert(Request request, Response response) {
	    ...
	    int nextId = produtoDAO.getNextId(); // Obtém o próximo ID disponível

	    Produto produto = new Produto(nextId, descricao, preco, quantidade, dataFabricacao, dataValidade);

	    if (produtoDAO.insert(produto)) {
	        resp = "Produto (" + descricao + ") inserido com sucesso!";
	        response.status(201); // 201 Created
	    } else {
	        resp = "Falha ao inserir o produto (" + descricao + ")";
	...
	public int nextId() {
	    ...
	    if (produtos.isEmpty()) {
	        return 1; // Se não houver produtos, retorna o ID inicial como 1
	    } else {
	        // Obtém o último ID da lista ordenada de produtos
	        Produto ultimoProduto = produtos.get(produtos.size() - 1);
	        // Incrementa o último ID e retorna o próximo ID disponível
